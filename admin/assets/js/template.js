(function ( $ ) {
    ...
    function addArgsBar(args, shortcode, iteration) {
        var argsBar;
        argsBar = '<div class"nc-button-bar--args">';
        ...
        for (var arg = 0; arg < args.length; arg++) {
            ...
            argsBar += '<input type="text" name="' + args[arg].name + '" id="' + args[arg].name + '" placeholder="Optional">';
            argsBar += '</div>';
        };

        argsBar += '<button type="button" class="button" id="nc-shortcode-arg-' + shortcode + '">Insert</button>';
        argsBar += '</div>';
